...
export default class CityGoodShopList extends Component {
                ...
                <View>
                    <FlatList
                        ...
                        renderItem={({item, index}) => {
                            if(this.index == 0 && this.state.dataSource) {
                                //商品列表
                                if(this.state.dataSource.length > 3) {
                                    return this._renderItem(item, index, this.state.totalNum);
                                }else {
                                    return this._renderItem2(item, index);
                                }
                            }else {
                                //店铺列表
                                return this._renderItem3(item, index);
                            }
                        }}
                        enableEmptySections={true}  //允许空数据
                        ListHeaderComponent={()=>this.pageTop(this.state.isFloat ? null : btnBox)}
                        onEndReached={()=>{
    ...
    _renderItem = (obj, rowID, num) => {
        ...
        boxStyle.height = _height;
        boxStyle.marginRight = _marginRight;
        if(_marginTop < 0) {
            // boxStyle.position = 'absolute';
            // boxStyle.top = _marginTop;
            boxStyle.transform = [{
                translateY: _marginTop,
            }];
        }

        return (
            <Animated.View key={rowID} style={boxStyle}>
                {ad}
                <ProductItem
                    key={rowID}
                    width={width}
